...
if ($from !== '')      { $where[] = "oi.created_at >= :from"; $args[':from'] = $from . ' 00:00:00'; }
if ($to !== '')        { $where[] = "oi.created_at <= :to";   $args[':to']   = $to . ' 23:59:59'; }
if ($talep_id !== '')  { $where[] = "oi.talep_id = :talep_id"; $args[':talep_id'] = $talep_id; }
if ($durum !== '')     { $where[] = "oi.durum = :durum"; $args[':durum'] = $durum; }
...
// Durum listesi
$durumlar = ['beklemede', 'teklif_alindi', 'onaylandi', 'siparis_verildi', 'teslim_edildi', 'iptal'];
?>
  ...
  <div class="card p-3">
        ...
        <div style="flex:1">
          ...
          <select name="durum">
            ...
            <?php foreach ($durumlar as $d): ?>
              <option value="<?= h($d) ?>" <?= $durum === $d ? 'selected' : ''; ?>>
                <?= h(ucfirst(str_replace('_', ' ', $d))) ?>
              </option>
            <?php endforeach; ?>
          </select>
        </div>
    ...
    <div class="table-responsive" style="margin-top:20px">
      <table class="table">
        ...
        <tbody>
          <?php if (!$rows): ?>
            <tr><td colspan="11" class="ta-center">Kayıt bulunamadı</td></tr>
          <?php else: 
            foreach ($rows as $r): 
              // Durum badge class
          ...
          ?>
            <tr>
              ...
              <td><?= h($r['birim'] ?: '-') ?></td>
              <td class="ta-right"><?= h(number_format((float)$r['birim_fiyat'], 2, ',', '.')) ?> ₺</td>
              <td class="ta-right"><strong><?= h(number_format((float)$r['tutar'], 2, ',', '.')) ?> ₺</strong></td>
              <td>
